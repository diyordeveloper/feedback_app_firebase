...
function SendMess() {
  ...
  const usersCollectionRef = collection(db, "feedback", id, "comment"); 
  ...
  const SendComment = async (e) => {
    ...
    e.preventDefault(); 
    if (text !== "") {
      await addDoc(collection(db, "feedback", id, "comment"), {
        timestamp:
          new Date().getHours() +
          ":" +
          new Date().getMinutes() +
  ...
  const deleteUser = async (idd) => {
    if (window.confirm("Delete data ? ")) {
      const userDoc = doc(db, "feedback", id, "comment", idd);
      await deleteDoc(userDoc);
      notify("Deleted");
    }
  };
  // const Edited = async (idb) => {
  //   const UserDoc = doc(db, "feedback", id, "comment", idb);
      ...
      <div className="sendcomment">
        ...
        <div className="d_flex">
          ...
          <Button
            ...
            variant="contained"
            color="primary"
            endIcon={<FaTelegramPlane className="telegram" />}
          >
            Post Comment
          </Button>
        </div>
